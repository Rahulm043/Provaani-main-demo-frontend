...
export default function AddCounsellorModal({ onSave, onClose, departments = [], courses = [] }) {
    ...
    async function handleSave() {
        ...
        setSaving(true);
        try {
            const res = await authFetch('/api/crm/councillors', {
                method: 'POST',
                body: JSON.stringify({
                    name: form.name.trim(),
            ...
            if (!res.ok) throw new Error(data.detail || 'Failed to create counsellor');
            onSave(data.councillor);
            setSaving(false);
            setSaved(true);
        ...
        <div className="cdash-detail-overlay" onClick={onClose}>
            <div className="acm-panel" onClick={e => e.stopPropagation()}>
                ...
                <div className="acm-form">
                    ...
                    {/* Stream (filtered by dept) */}
                    {form.category_id && (
                        <FieldWrap icon={BookOpen} label="Assigned Stream" required>
                            <select
                                className={`acm-select ${errors.course_id ? 'acm-input--error' : ''}`}
                            ...
                            >
                                ...
                                {filteredCourses.map(c => (
                                    <option key={c.id} value={c.id}>
                                        {c.stream_name || c.course_name || c.name}{c.duration_years ? ` (${c.duration_years}yr)` : ''}
                                    </option>
                                ))}
                            </select>
