            ...
            <div className="inspector-panel">
                ...
                {tab === 'overview' && (
                    <div>
                        ...
                        {/* AI Insights Section */}
                        {(call.ai_extracted_data || (call.metadata && call.metadata.ai_analysis)) && (
                            <div className="inspector-section">
                                <h4>AI Insights</h4>
                                <div className="detail-grid">
                                    {Object.entries(call.ai_extracted_data || call.metadata.ai_analysis || {})
                                        .filter(([k]) => k !== 'transcript')
                                        .map(([k, v]) => (
                                            <div key={k} className="detail-item">
                                                <div className="detail-label">{k.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}</div>
                                                <div className="detail-value">
                                                    {typeof v === 'boolean' 
                                                        ? (v ? '✅ Yes' : '❌ No') 
                                                        : (Array.isArray(v) ? v.join(', ') : (typeof v === 'object' && v !== null ? JSON.stringify(v) : (v || '—')))}
                                                </div>
                                            </div>
                                        ))}
                                </div>
